 ...
 * 数据打开
 */
class SDT670 extends ModbusBase {
    ...
    ReadWQ (mapItem){
        ...
        return this.CreateWQReader(mapItem,function(reg,results){
            ...
            switch (reg) {
                case 1:  //电量
                    return this.mbClient.readInputRegisters(10, 2).then(function(newData){
                        results.push( (((newData.data[1]<<16)+newData.data[0])/100.0).toFixed(4));
                    });
                    break;
                    break;
                case 2:  //电压
                    return this.mbClient.readInputRegisters(50, 3).then(function(newData){
                        results.push( {
                ...
                case 3: //电流
                    ...
                    return this.mbClient.readInputRegisters(56, 3).then(function(newData){
                        results.push( {
                            A:(newData[0]/100).toFixed(2),
                            B:(newData[1]/100).toFixed(2),
                            C:(newData[2]/100).toFixed(2),
